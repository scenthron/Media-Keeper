...
def save_dump(
    ...
    # Save settings
    cur.execute(
        "INSERT OR REPLACE INTO settings (key, value) VALUES (?, ?)", ("type", search_type))
    cur.execute("INSERT OR REPLACE INTO settings (key, value) VALUES (?, ?)",
                ("is_hash_only", "1" if is_hash_only else "0"))

    # Save images if not hash only
    if not is_hash_only:
        for p_path in pos_image_paths:
            if os.path.exists(p_path):
                ...
                cur.execute(
                    "INSERT INTO images (is_positive, filename, data) VALUES (?, ?, ?)", (1, filename, data))
        ...
        for n_path in neg_image_paths:
            if os.path.exists(n_path):
                ...
                cur.execute(
                    "INSERT INTO images (is_positive, filename, data) VALUES (?, ?, ?)", (0, filename, data))

    # Save features
    for feat in pos_features:
        feat_bytes = feat.tobytes()
        # Save shape to settings if needed, but YOLO and SigLIP features are 1D arrays usually
        shape_str = ','.join(map(str, feat.shape))
        dtype_str = str(feat.dtype)
        # We can prepend shape and dtype to the blob or assume it based on search_type
        # To be safe, we can store it in the blob: shape_str + "|" + dtype_str + "|" + feat_bytes
        header = f"{shape_str}|{dtype_str}|".encode('utf-8')
        full_blob = header + feat_bytes
        cur.execute(
            "INSERT INTO features (is_positive, feature_vector) VALUES (?, ?)", (1, full_blob))
    ...
    for feat in neg_features:
        ...
        full_blob = header + feat_bytes
        cur.execute(
            "INSERT INTO features (is_positive, feature_vector) VALUES (?, ?)", (0, full_blob))
    ...
    try:
        ...
        for is_pos, filename, data in cur.fetchall():
            out_path = os.path.join(
                temp_dir, f"{'pos' if is_pos else 'neg'}_{filename}")
    ...
    try:
        ...
        for is_pos, filename, data in cur.fetchall():
            sub_dir = os.path.join(
                dest_dir, "positives" if is_pos else "negatives")
        ...
        logging.error(
            f"Error extracting images from {path} to {dest_dir}: {e}")
    ...
    try:
        ...
        for is_pos, blob in cur.fetchall():
            try:
                ...
                arr = np.frombuffer(
                    feat_bytes, dtype=np.dtype(dtype_str)).reshape(shape)
