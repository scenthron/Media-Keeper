...
class CleanerSelectionMixin:
    ...
    def select_duplicates_of_source(self, file_path: str) -> None:
        self.overlay.start_loading_mode(0)
        self.overlay.lbl_title.setText("Выделение копий...")
        QCoreApplication.processEvents()
        QTimer.singleShot(20, lambda: self._select_duplicates_of_source_impl(file_path))

    def _select_duplicates_of_source_impl(self, file_path: str) -> None:
        self._load_cancelled = False
        self.overlay.btn_stop.hide()
        try:
            if not hasattr(self, 'in_memory_selection'):
            ...
            # Find the source root of the clicked file
            source_root = None
            is_dump_item = file_path.startswith('[Дамп]')
            
            if is_dump_item:
                # For dumps, the virtual model 'path' is exactly the dump path string
                source_root = file_path
            else:
                for src in self.source_folders.keys():
                    if is_subpath(file_path, src):
                        source_root = src
                        break
            ...
            for item in self.virtual_model._all_items:
                if item['type'] == 'file':
                    ...
                    if is_dump_item:
                        if item_path == source_root:
                            target_group_ids.add(item['group_id'])
                    else:
                        if is_subpath(item_path, source_root):
                            target_group_ids.add(item['group_id'])
