...
class MovePreviewDialog(QDialog):
    def __init__(self, mapping: list[dict], parent=None):
        super().__init__(parent)
        self.setWindowTitle(AppContext.tr("cln_preview_title") if hasattr(AppContext, 'tr') else "Подтверждение перемещения")
        self.resize(700, 500)
        self.setStyleSheet('''
            QDialog { background-color: #2b2b2b; color: white; }
            QLabel { color: #cccccc; font-size: 12px; }
            QPushButton { background-color: #444; border: 1px solid #555; color: white; border-radius: 4px; padding: 6px 15px; font-weight: bold; }
            QPushButton:hover { background-color: #555; }
            QScrollArea { border: 1px solid #333; background: #1e1e1e; border-radius: 4px; }
        ''')
        ...
        for item in mapping:
            ...
            frame = QFrame()
            frame.setStyleSheet("background: #252526; border-radius: 4px; padding: 4px;")
            fl = QVBoxLayout(frame)
            fl.setContentsMargins(5, 5, 5, 5)
            
            l_src = QLabel(f"Из: {src}" if AppContext.LANG == "RU" else f"From: {src}")
            l_src.setStyleSheet("color: #999; font-size: 11px;")
            l_dst = QLabel(f" В: {dst}" if AppContext.LANG == "RU" else f"  To: {dst}")
            l_dst.setStyleSheet("color: #4ade80; font-weight: bold;")
        ...
        btn_cancel = QPushButton("Отмена" if AppContext.LANG == "RU" else "Cancel")
        btn_cancel.clicked.connect(self.reject)
        
        btn_apply = QPushButton("Продолжить" if AppContext.LANG == "RU" else "Proceed")
        btn_apply.setStyleSheet("background-color: #3b82f6; border: none;")
        btn_apply.clicked.connect(self.accept)
