...
class AiScanWorker(QThread):
    ...
    def run(self):
        ...
        # ЭТАП 1: Сканируем файлы и строим кэш (СИНИЙ ЭТАП)
        valid_files = []
        scanned_files = 0
        scanned_bytes = 0
        ...
        # ЭТАП 2: ПОИСК СОВПАДЕНИЙ / КЛАСТЕРИЗАЦИЯ (Зеленый прогресс-бар)
        results = {}
        processed_files = 0
        groups_found = 0
        files_found = 0
            ...
            try:
                ...
                if getattr(self, "is_cluster", False):
                    ...
                    if getattr(self, "cluster_type", "face") == "face":
                        ...
                        emb = None
                        if self.use_cache:
                            emb = self.classifier.cache.get_image_embedding(fp, mtime, size)
                        if emb is None:
                            emb = self.classifier.ai.extract_clip_embedding(fp)
                            if self.use_cache and emb is not None:
                                self.classifier.cache.save_image_embedding(fp, mtime, size, emb)
